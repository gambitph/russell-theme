...
function russell_scripts() {
	...
	if ( ! class_exists( 'TitanFramework' ) ) {
		wp_register_style( 'russell-font-montserrat', 'http://fonts.googleapis.com/css?family=Montserrat:400,700' );
		wp_register_style( 'russell-font-roboto', 'http://fonts.googleapis.com/css?family=Roboto+Slab:300,700' );
		wp_enqueue_style( 'russell-font-montserrat' );
...
function russell_large_content( $num = 10, $page = 1, $query = null, $tagID = null ) {
	...
	if ( class_exists( 'TitanFramework' ) ) {
		...
		$enable = $titan->getOption( 'display_post' );
		if ( $enable == false ) {
